...
| [4.2](part4-metadata.qmd#section4-2) | Controlled Vocabularies | Defines the controlled vocabularies to be applied in metadata fields. Includes Developmental Stage Class Codes ([Table 4.4](#tbl-4-4)), Target Class Codes ([Table 4.5](#tbl-4-5)), Tissue Descriptor Codes ([Table 4.6](#tbl-4-6)), Background and White Reference Class Codes ([Table 4.7](#tbl-4-7)), and Specimen Identifier Class Codes ([Table 4.8](#tbl-4-8)). |
...
The three tables below describe the session, specimen, and tissue metadata fields. 

The [IHerbSpec Metadata Spreadsheet](tables/IHerbSpec_metadata.csv) also includes the “Filename” and "simpleFilename" columns. These have been populated with required filename codes as prefixes (e.g., `PI`, `SN`, `SI`, `TC`) separated by underscores (see [Part 3](part3-filenames.qmd)).
...
Specimen metadata include identifiers and information about the physical specimen, with priority given to **required** fields needed to link spectral measurements to existing digital records. The `specimenId` field records the identifier used for this linkage, while `specimenIdClass` records the type of identifier using controlled vocabulary codes [Table 4.8](#tbl-4-8) or concise free text when necessary. Optional fields related to taxonomic determination and specimen storage environment are included to support integrative research, quality control, and downstream analysis.
...
| `specimenId` | `SI` | Required | Identifier used to link the spectral/trait metadata record to the physical specimen or to the institution’s specimen record. This field should contain the identifier most appropriate for that collection, such as a GUID, GBIF occurrence ID, barcode, catalog number, accession number, or collector name and number.<br> Examples: `00238762`, `Thorne24070a` | TEXT |
| `specimenIdClass` | - | Required | Class code describing the type of identifier provided in `specimenId`. Use an enumerated code from [Table 4.8](#tbl-4-8) when possible; otherwise, provide a concise text description of the identifier class.<br> Examples: `BarcodeID`, `UUID`, `CollectorNameAndNumber` | ENUM<br> ([Table 4.8](#tbl-4-8))<br> or TEXT |
...
| `targetClass`                 | `TC` | Required | Free text or enumerated code from Target Class Codes ([Table 4.5](#tbl-4-5)) describing type of tissue or background being measured. Both abbreviated codes and full codes can be used.<br> Examples: `AD`, `perigynium`       | ENUM<br> ([Table 4.5](#tbl-4-5))<br> or TEXT |
...
### Table 4.8: Specimen Identifier Class Codes {#tbl-4-8}

Available codes for the required `specimenIdClass` metadata field ([Table 4.2](#tbl-4-2)). Use these codes when possible; otherwise, provide a concise free-text class using CamelCase format.

| Code | Guidance | Definition |
|---|---|------------------|
| `UUID` | Recommended | A universally unique identifier assigned to the specimen, specimen fragment, or related Darwin Core occurrence record. UUIDs are 32-digit identifiers constructed according to the RFC 9562 standard; they may also be referred to as GUIDs. UUIDs should be persistent, preserved in institutional databases, and intended for long-term use. It is also permissible to format a UUID as a resolvable URL. Examples for `specimenId` entry: `7783cd80-2e3e-49cb-bb9b-95f1c7d2141f`, `https://data.huh.harvard.edu/7783cd80-2e3e-49cb-bb9b-95f1c7d2141f` |
| `BarcodeId` | Recommended | A specimen barcode number or identifier assigned by the specimen’s home institution to identify and track the physical specimen. Barcode identifiers should be stable, preserved in institutional databases, and intended for long-term use. They are expected to be the most frequently used identifier class because they can often be entered efficiently and accurately using a barcode scanner. Example for `specimenId` entry: `00679514` |
| `PersistentId` | Accepted | Any persistent, externally meaningful identifier that does not conform to the UUID standard. These identifiers should be preserved in institutional, aggregator, or repository databases and intended for long-term use. URI form is recommended to ensure uniqueness. This class may include GUIDs that are not UUIDs, Darwin Core `occurrenceID` values, ARKs, URNs, or GBIF identifiers when the GBIF occurrence record is the intended external linkage target. Examples for `specimenId` entry: `urn:catalog:MO:Tropicos:105059003`, `http://n2t.net/ark:/65665/3e637e7c9-8454-4785-98cb-3fed71df7654`, `B7C40E4A06465C9795AF9DF1B6BD342C.mc.31640B656ECDBCBE6579AD770ED363DB`, `5085584305` |
| `CatalogNumber` | Accepted | Any identifier apart from the specimen barcode used by the specimen’s home institution to catalog and locate the physical specimen, including internal reference numbers (IRN). Example for `specimenId` entry: `2032448` |
| `CollectorNameAndNumber` | Accepted | The concatenated collector name and number assigned to the specimen. Recommended format for `specimenId` entry is collector last name, or concatenated collector last names, with each name beginning with a capital letter, followed by capitalized first-name initials, an underscore, and the collector number or code. Examples for `specimenId` entry: `BouffordDE_123`, `RodriguezSmithE_1999-23` |
| `AccessionNumber` | Accepted | Any identifier related to the accessioning of the physical specimen. |
| `TransientId` | Discouraged | An identifier that uniquely identifies a specimen only within a single spectral dataset. This identifier cannot be used to link specimen data with other datasets or databases. |
| `LocalDatabaseId` | Discouraged | An identifier used for the specimen within an institutional or personal database. This identifier may not be externally resolvable or intended for long-term persistence. |
...
- In addition to institutional archiving, projects are strongly encouraged to **deposit unprocessed spectral data files and a metadata spreadsheet in a persistent, open-access repository** that issues DOIs and supports versioning.  
...
- To enhance discoverability and specimen-level integration, datasets archived in external repositories (including the IHerbSpec Dataverse) may be linked to specimen records using standardized associations (e.g., Darwin Core Resource Relationship terms such as `isReferencedBy`). These links can be prepared in bulk and ingested by specimen portals, allowing users to search for specimens associated with spectral data while the files themselves remain hosted in the most appropriate repository.

- To facilitate reuse and analysis, projects may also share **tabular data files** (e.g., `.csv`) containing samples in rows and spectral bands and metadata fields in columns. If processed spectra are included (e.g., interpolated, splice-corrected), all applied processing steps should be clearly documented in the data archive description.
